...
def check_login():
    ...
    st.markdown(f'<div style="display:grid;grid-template-columns:1fr 1fr;gap:0.6rem;margin-bottom:1rem"><div style="background:{BG2};border:1px solid {BD};border-left:4px solid {AC};border-radius:10px;padding:0.9rem"><b style="color:{AC2};font-size:1.15rem">🛡️ Always Accurate</b><ul style="list-style:none;padding:0;margin:0.4rem 0 0"><li style="padding:0.15rem 0;font-size:0.95rem;color:{CTX}">✓ Tax numbers from a precise calculation engine — not AI guesses</li><li style="padding:0.15rem 0;font-size:0.95rem;color:{CTX}">✓ Every answer cites the actual Income Tax Act section</li><li style="padding:0.15rem 0;font-size:0.95rem;color:{CTX}">✓ TaxGuru says "check with a CA" when unsure — never makes things up</li><li style="padding:0.15rem 0;font-size:0.95rem;color:{CTX}">✓ Updated real-time with Budget 2026 changes, circulars, court judgements</li></ul></div><div style="background:{BG2};border:1px solid {BD};border-left:4px solid {AC};border-radius:10px;padding:0.9rem"><b style="color:{AC2};font-size:1.15rem">🔒 Your Data Stays Private</b><ul style="list-style:none;padding:0;margin:0.4rem 0 0"><li style="padding:0.15rem 0;font-size:0.95rem;color:{CTX}">✓ We never see your PAN, Aadhaar, address, DOB, phone, or email</li><li style="padding:0.15rem 0;font-size:0.95rem;color:{CTX}">✓ Bank accounts, PF/UAN, employer name — all auto-deleted</li><li style="padding:0.15rem 0;font-size:0.95rem;color:{CTX}">✓ Only salary/deduction numbers used — in your browser session only</li><li style="padding:0.15rem 0;font-size:0.95rem;color:{CTX}">✓ Close the browser → everything gone. Nothing saved. Ever.</li></ul></div></div>',unsafe_allow_html=True)
    if has_db:
...
SHORT="""You are {agent_name}, a friendly and knowledgeable Indian income tax advisor at TaxGuru.
Answer naturally like a helpful expert would — be conversational but accurate.
Cite specific section numbers when relevant (e.g., "Section 80C", "Section 112A").
Keep answers focused and practical — what should the user DO, not just theory.
If you have access to their tax profile data, reference their specific numbers.
For product recommendations (mutual funds, insurance, loans), give specific names and details available in India.
Do not add disclaimers about consulting a CA — that's already shown on the page."""
    ...
    def tp():
        ...
        if up and K:
            ...
            if 'error' not in doc:
                ...
                # Store extracted values in session so callback can access them
                st.session_state._doc_gs=gv('gross_salary')
                st.session_state._doc_bs=gv('basic_salary',gv('gross_salary')*0.4)
                st.session_state._doc_hra=gv('hra')
                st.session_state._doc_tds=gv('tds_deducted')
                st.session_state._doc_80c=min(gv('section_80c_total',gv('pf_employee')),150000)
                st.session_state._doc_nps=gv('section_80ccd_2',gv('pf_employer'))
                def _use():
                    p=P();p.taxpayer_type="salaried"
                    p.gross_salary=st.session_state.get('_doc_gs',0)
                    p.basic_salary=st.session_state.get('_doc_bs',0)
                    p.hra_received=st.session_state.get('_doc_hra',0)
                    p.tds_deducted=st.session_state.get('_doc_tds',0)
                    p.section_80c=st.session_state.get('_doc_80c',0)
                    p.section_80ccd_2=st.session_state.get('_doc_nps',0)
                    st.session_state.pc=True;save_all();nav("Tax Calculator")
